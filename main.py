...
def main():
    ...
    train_ep_score_hist =[]
    test_ep_score_hist =[]
    train_ep_uncertainty_reward_hist =[]
    ...
    if args.behavior_cloning or args.dril or args.warm_start:
        ...
        bc_file_name = f'bc_{args.env_name}_policy_ntrajs={args.num_trajs}_seed={args.seed}_d={args.distribution}'
    ...
    for j in range(num_updates):
        ...
        for step in range(args.num_steps):
            ...
            for i, info in enumerate(infos):
                if 'episode' in info.keys():
                    ...
                    train_ep_score_hist.append(info['episode']['r'])
                    train_ep_uncertainty_reward_hist.append(running_uncertainty_reward[i] / info['episode']['l'])
        ...
        if (j % args.save_interval == 0
            ...
            model_file_name = f'{args.env_name}_policy_ntrajs={args.num_trajs}_seed={args.seed}_d={args.distribution}'
            torch.save([
                actor_critic,
        ...
        if (args.eval_interval is not None and len(episode_rewards) > 1 and
            (j % args.eval_interval == 0 or np.mean(episode_rewards) > best_training)):
            ...
            file_name = f'{algo}_{args.env_name}_policy_ntrajs={args.num_trajs}_seed={args.seed}_d={args.distribution}_vc={args.value_loss_coef}' \
                        f'_elc={args.entropy_coef}_clip={args.clip_param}_steps={args.num_steps}_lr={args.lr}_decay={args.use_linear_lr_decay}_scr={int(test_reward)}'


            if test_reward > best_score:
                print(f'{"*" * 30} \n Saving model at {save_path + "/" + file_name} \n{"*" * 30}')
                best_score = test_reward
                best_training = np.mean(episode_rewards)
                print(f'New training threshold = {best_training}, New testing threshold ={best_score}')
                torch.save([
                    actor_critic,
                    getattr(utils.get_vec_normalize(envs), 'ob_rms', None),
                    args,
            ...
            file_name_h = f'{algo}_{args.env_name}_policy_ntrajs={args.num_trajs}_seed={args.seed}_d={args.distribution}' \
                          f'_vc={args.value_loss_coef}_elc={args.entropy_coef}_clip={args.clip_param}_steps={args.num_steps}' \
                          f'_lr={args.lr}_decay={args.use_linear_lr_decay}_training'
            ...
            torch.save([
                ...
                train_ep_score_hist,
                train_ep_uncertainty_reward_hist
            ], os.path.join(save_path, f"{file_name_h}.pt"))
...
def save_results(args, main_results, algo, dril, gail):
    ...
    if dril:
        ...
        exp_name += f'cost_{args.dril_cost_clip}_seed={args.seed}_d={args.distribution}.perf'
