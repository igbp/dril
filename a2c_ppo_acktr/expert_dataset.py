...
class ExpertDataset:
    ...
    def load_demo_data(self, training_data_split, batch_size, ensemble_size):
        ...
        acs_file = f'{self.demo_data_dir}/{args.distribution}/acs_{self.env_name}_seed={self.seed}_ntraj={self.num_trajs}.npy'

        print(f'loading: {acs_file}')
        sample = np.load(acs_file)
        print(f'Actions shape: {sample.shape}')
        print(f'10 Acs sample: {sample[0:10]}')
