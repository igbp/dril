...
class DimReductorGaussian(gym.ActionWrapper):
...
class DimReductorBeta(gym.ActionWrapper):
    def __init__(self, env):
        super().__init__(env)
        self.action_space = gym.spaces.Box(low=np.array([0.0, 0.0]), high=np.array([1.,1.]))

    def action(self, action_in):
        action_out = np.zeros(3)
        action_out[0] = (action_in[0] - 0.5) * 2.0
        action_out[1] = np.maximum((action_in[1] - 0.5) * 2.0, 0)
        action_out[2] = np.maximum(-(action_in[1] - 0.5) * 2.0, 0)

        return action_out

class DimBeta_LL(gym.ActionWrapper):
    def __init__(self, env):
        super().__init__(env)
        self.action_space = gym.spaces.Box(low=np.array([0.0, 0.0]), high=np.array([1.,1.]))

    def action(self, action_in):
        action_out = (action_in - 0.5)*2.0
        return action_out
...
def make_env(env_id, seed, rank, log_dir, allow_early_resets, time=False, max_steps=None):
    def _thunk():
        ...
        args = get_args()
        if args.env_name == "LunarLanderContinuous-v2" and args.distribution == 'beta':
            env = DimBeta_LL(env)
            #print("Using Beta distribution for LunarLanderContinuous-v2")
        ...
        if is_atari:
            ...
            if env_id not in ['duckietown'] and env_id not in retro_envs:
                ...
                args = get_args()
                if args.distribution == 'discrete' and args.env_name == 'CarRacing-v0':
                ...
                if args.distribution == 'gaussian' and args.env_name == 'CarRacing-v0':
                    env = DimReductorGaussian(env)

                if args.distribution == 'beta' and args.env_name == 'CarRacing-v0':
